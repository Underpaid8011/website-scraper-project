...
def build_command(base_url):
    script_path = Path(__file__).resolve().parent / "scraper.py"
    if not script_path.exists():
        raise FileNotFoundError(f"Could not find scraper.py at {script_path}")
    return [sys.executable, "-u", str(script_path), "--base-url", base_url]
